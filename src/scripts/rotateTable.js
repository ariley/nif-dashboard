...
document.addEventListener('DOMContentLoaded', () => {
			...
			rows = sortedData.map((row) => {
				...
				return {
					...
					html: `<tr class="text-xs sm:text-sm px-2">
							...
							<td class="dark:border-gray-700 px-2 column-4 portrait:hidden">${row[3]}</td>
							<td class="dark:border-gray-700 px-2 column-5 portrait:hidden">${formattedDate1}</td>
	...
	function formatExcelDate(excelDate) {
		...
		if (!Number.isNaN(excelDate)) {
			...
			const date = new Date(
				(excelDate - (excelDate > 59 ? 25569 : 25568)) * 86400 * 1000
			);
